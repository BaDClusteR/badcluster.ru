...
export class TerminalBlock implements BlockTool {
  ...
  private readonly data: TerminalBlockData;
  private wrapper!: HTMLElement;
  private contentEls: Partial<Record<TabKey, HTMLElement>> = {};
  private activeTab: TabKey = "cipher";

  constructor({data}: { data: BlockToolData<TerminalBlockData>; api: API }) {
    this.data = {
      cipher: data?.cipher ?? "",
      en: data?.en ?? "",
      ru: data?.ru ?? "",
  ...
  renderSettings(): HTMLElement {
    ...
    wrapper.appendChild(separator());
    wrapper.appendChild(heading("Вкладки"));
